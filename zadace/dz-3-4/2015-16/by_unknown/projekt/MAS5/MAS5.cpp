...
int main(int argc, char *argv[]) {
	...
	//char* datotekaizlaz = argv[2];
	char* datotekaizlaz = "out.txt";
	int x;
	...
	for (x = 0; x<4096; x++){ // za sve blokove!
		blok = x;
		int vertikala = 0;
		int pomak = 0;
		pomak = (blok / 64);
		...
		ippiDCT8x8Fwd_32f_C1I(Y); //IPP
		ippiDCT8x8Fwd_32f_C1I(Cb); //IPP
		ippiDCT8x8Fwd_32f_C1I(Cr); //IPP
		...
		*/
		...
		for (i = 0; i<8; i++) {
			for (j = 0; j<8; j++) {
				KY[i][j] = (round)(Y[i][j] / Ktablica1[i][j]); //IPP
				KCb[i][j] = (round)(Cb[i][j] / Ktablica2[i][j]); //IPP
				KCr[i][j] = (round)(Cr[i][j] / Ktablica2[i][j]); //IPP
			}
		}
